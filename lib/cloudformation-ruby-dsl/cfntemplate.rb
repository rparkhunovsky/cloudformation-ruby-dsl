...
def cfn(template)
  ...
  immutable_tags = template.excise_tag_attribute!(cfn_tags, :Immutable)

  # Transform tags properties hash to `tag => value` format.
  cfn_tags.each {|k, v| cfn_tags[k] = v[:Value]}
  ...
  when 'diff'
    ...
    if !tags_diff.empty?
      ...
      puts tags_diff
      puts "=================="
      puts "CloudFormation stack tags do not match and are to be updated."
      puts
    end
    ...
    tags_diff.empty? && params_diff.empty? && template_diff.empty? ? exit(true) : exit(2)
  ...
  when 'update'
    ...
    old_cfn_tags = old_stack.tags.map { |t| [t.key.to_sym, t.value]}.sort
    cfn_tags_ary = cfn_tags.map { |k,v| [k.to_sym, v]}.sort
    ...
    # Fail if updating a stack and some tags are marked as immutable.
    if not immutable_tags.empty?
      old_cfn_tags = Hash[old_cfn_tags.map { |k,v| [k, v]}]
      cfn_tags_ary = Hash[cfn_tags_ary.map { |k,v| [k, v]}]
      immutable_tags.sort.each do |tag|
        if old_cfn_tags[tag].to_s != cfn_tags_ary[tag].to_s
